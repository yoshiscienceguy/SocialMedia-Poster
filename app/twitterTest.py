...
def twitter_get_oauth_token(ro_key, ro_secret):

    

    request_token = OAuth1Session(client_key=consumer_key, client_secret=consumer_secret)
    url = 'authorization_url = f"https://api.twitter.com/oauth/authorize?oauth_token={ro_key}"'
    data = request_token.get(url)
...
resource = twitter_get_oauth_request_token()
twitter_get_oauth_token(resource[0],resource[1])
